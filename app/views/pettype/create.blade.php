...
<h1>Create a Pet Type</h1>

<!-- if there are creation errors, they will show here -->
{{ HTML::ul($errors->all()) }}

{{ Form::open(array('url' => 'pettype')) }}

    <div class="form-group">
        {{ Form::label('pettype', 'Pet Type') }}
        {{ Form::text('pettype', Input::old('pettype'), array('class' => 'form-control')) }}
    </div>

    <div class="form-group">
        {{ Form::label('status', 'Pet type Status') }}
        {{ Form::select('status', array('0' => 'Inactive', '1' => 'Active')) }}
    </div>

    {{ Form::submit('Create new Pet Type!', array('class' => 'btn btn-primary')) }}
