...
<br/>
<h1>Edit {{ $pettype->pettype }}</h1>

<!-- if there are creation errors, they will show here -->
{{ HTML::ul($errors->all()) }}

{{ Form::model($pettype, array('route' => array('pettype.update', $pettype->id), 'method' => 'PUT')) }}

    <div class="form-group">
        {{ Form::label('pettype', 'Pet Type') }}
        {{ Form::text('pettype', null, array('class' => 'form-control')) }}
    </div>

    <div class="form-group">
        {{ Form::label('status', 'Service Status') }}
        {{ Form::select('status', array('0' => 'Inactive', '1' => 'Active'), null, array('class' => 'form-control')) }}
    </div>

    {{ Form::submit('Edit Pet Type!', array('class' => 'btn btn-primary')) }}
